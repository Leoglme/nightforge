    ...
    <div class="flex min-h-dvh">
      ...
      >
        <div class="flex items-center gap-2.5 px-6 pt-6 md:px-10">
          ...
          <span class="text-sm font-semibold tracking-tight text-[var(--app-ink)]">{{ t('app.name') }}</span>
        </div>
            ...
            <form class="mt-10 space-y-5" @submit.prevent="onSubmit">
              ...
              <div class="auth-rise" :style="{ animationDelay: '350ms' }">
                ...
                <div class="relative w-full">
                  <input
                    ...
                    :class="['app-input pr-10', passwordError && 'app-input--error']"
                  />
                  <button
                    type="button"
                ...
                <p v-if="passwordError" class="mt-1.5 text-xs text-[var(--app-red)]">{{ passwordError }}</p>
              </div>
              ...
              <div class="auth-rise pt-2" :style="{ animationDelay: '420ms' }">
                <button type="submit" :disabled="isLoading" class="app-btn-primary w-full">
                  ...
                  <span>{{ isLoading ? t('auth.signup.submitting') : t('auth.signup.submit') }}</span>
                </button>
              </div>

              <p class="auth-rise text-center text-sm text-[var(--app-ink-soft)]" :style="{ animationDelay: '490ms' }">
...
/**
 * Signup page — split-screen "affiche de nuit" auth shell (dark only).
 */
...
/**
 * Submit the signup form (client-side password length check mirrors the API).
 * @returns Nothing.
 */
